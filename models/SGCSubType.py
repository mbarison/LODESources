...
class SGCSubType(Base):
    ...
    sgc_type_uid: Mapped[int] = mapped_column(ForeignKey(StandardGeographicClassificationLevel.sgc_type_uid), primary_key=True)
    ...
    def sgc_subtype_name(self) -> str:
        return self.sgc_subtype_name_en if self.sgc_subtype_name_en else self.sgc_subtype_name_fr

    def __repr__(self):
        return "<SGCSubType(sgc_type_uid='%s', sgc_subtype_uid='%s', sgc_type='%s',  sgc_subtype_name_en='%s', sgc_subtype_name_fr='%s')>" % (
            self.sgc_subtype_uid, self.sgc_type_uid, self.sgc_type, self.sgc_subtype_name_en, self.sgc_subtype_name_fr)