...
class StandardGeographicClassification(Base):
    ...
    sgc_type_uid: Mapped[int] = mapped_column(
        ForeignKey(StandardGeographicClassificationLevel.sgc_type_uid)
    )
    sgc_subtype_uid: Mapped[int] = mapped_column()
    __table_args__ = (
        ForeignKeyConstraint(
            [sgc_type_uid, sgc_subtype_uid],
            [SGCSubType.sgc_type_uid, SGCSubType.sgc_subtype_uid],
        ),
    )
    ...
    def __repr__(self):
        return (
            "<StandardGeographicClassification(dguid='%s', sgc_uid='%s', sgc_name_en='%s', sgc_name_fr='%s', sgc_type_uid='%s', sgc_subtype='%s', sgc_level='%s')>"
            % (
                self.dguid,
                self.sgc_uid,
                self.sgc_name_en,
